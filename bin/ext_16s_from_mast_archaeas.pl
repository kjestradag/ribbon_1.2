...
my $mast= shift; # genome_windows.mast (resultado del mast)
my $org= shift; # $dir
...
sub ext_data_from_mast{
    ...
    while(<IN>){
        chomp;
        #~ if( /^(\S+)\.(\d+)\s+([\+\-])(\d+)\s+(\d+)\s+(\d+)/ ){ # mast -v == older
...
sub extract_16S{
    ...
    foreach my $filcont ( sort keys %fildata ){
        ...
        foreach my $filpart ( sort keys %{$fildata{$filcont}} ){
           ...
           foreach my $filpart_motif ( @{$fildata{$filcont}{$filpart}{motif}} ){
                $pos++;
                $bad= 0 if ($filpart_motif ==  '2');
            }

            $fildata{$filcont}{$filpart}{strand} eq "+" ? (print RIB ">${filcont}_$i\t$from..$to\t$strand\n", substr($seq->{$filcont}, $from, $length16s),"\n") : (print RIB ">${filcont}_$i\t$from..$to\t$strand\n", reverse_seq(substr($seq->{$filcont}, $from, $length16s)),"\n") if ($length16s >= 100 && $length16s <= 2000 && @{$fildata{$filcont}{$filpart}{motif}}> 3 && !$bad); #if ($length16s >= 1000 && $length16s <= 3000 && @{$fildata{$filcont}{$filpart}{motif}}> 3 && $doubleend <= 4 && $bad);

            $fildata{$filcont}{$filpart}{strand} eq "+" ? (print RIB2 ">${filcont}_$i\t$from..$to\t$strand\n", substr($seq->{$filcont}, $from, $length16s),"\n") : (print RIB2 ">${filcont}_$i\t$from..$to\t$strand\n", reverse_seq(substr($seq->{$filcont}, $from, $length16s)),"\n") if ($length16s >= 200 && $length16s <= 2000 && @{$fildata{$filcont}{$filpart}{motif}}> 3 && $bad); #if ($length16s >= 1000 && $length16s <= 3000 && @{$fildata{$filcont}{$filpart}{motif}}> 3 && $doubleend <= 4 && $bad);
        }
    }
}
